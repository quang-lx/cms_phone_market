<template>
    <div class="row">
        <div v-for="(group, index) in groupPermissions" :key="'g-' + index" class="col-md-4"
            ...
            <div class="card" style="height:97%">
                ...
                <div class="card-body">
                    <div class="row" v-for="(permission, pIndex) in group" :key="'p-'+pIndex">
                        <div class="col-3"><label class="control-label text-right"
                                                  style="text-transform: capitalize;display: block;">{{permission.title}}</label>
                        </div>
                        <div class="col-9">
                            <el-radio-group v-model="permissions[permission.name]" size="mini">
                                <el-radio-button :label="1" @click="triggerEvent(permission.name, 1)">{{
                                    $t('role.allow') }}
                                </el-radio-button>
                                <el-radio-button :label="-1" @click="triggerEvent(permission.name, -1)">{{
                                    $t('role.deny') }}
  ...
  export default {
    ...
    methods: {
      queryServer(customProperties) {
        ...
        axios.get(route('api.permissions.all-by-group', _.merge({ module: 'admin'}, customProperties)))
        .then((response) => {
