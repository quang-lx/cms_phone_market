                ...
                <div class="row mb-2">
                    <div class="col-12">
                        <el-breadcrumb separator="/">
                            ...
                            <el-breadcrumb-item :to="{ name: 'admin.shiptype.index' }"
                            >{{ $t("shiptype.label.shiptype") }}
                            </el-breadcrumb-item>
    ...
    methods: {
      onSubmit() {
        ...
        .then((response) => {
          ...
          this.$router.push({name: "admin.shiptype.index"});
        })
        .catch((error) => {
          this.loading = false;
          this.$notify.error({
      ...
      fetchData() {
        let routeUri = "";
        if (this.$route.params.shiptypeId !== undefined) {
          this.loading = true;
          routeUri = route("api.shiptype.find", {
            shiptype: this.$route.params.shiptypeId,
          });
      ...
      getRoute() {
        if (this.$route.params.shiptypeId !== undefined) {
          return route("api.shiptype.update", {
            shiptype: this.$route.params.shiptypeId,
          });
        }
        return route("api.shiptype.store");
      },
    },
    mounted() {
      this.fetchData();
