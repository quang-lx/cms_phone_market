<template>
    ...
    <section class="w-920 flex wrap-between node5-1">
        <div v-for="(item, index) in eventItems" :key="index" class="bg-2 w-452 mb-19 node5-16" @click="goToDetail(item)">
            ...
            <div class="flex row start-start pt-12 pb-16 pr-16 pl-16">
                ...
                <div class="offset-w-48 ml-16">
                    ...
                    <slot v-if="item.type === 'NORMAL IMAGE'" >
                        <div class="mt-4 flex row start-center">
                            <span class="ml-4 color-7 txt-14-22">{{ trans('base.News') }}</span>
                        </div>

                        <button class="h-32 w-134 flex col center-center bg-4 mt-8">
                            <span class="white txt-14-17 weight-bold">{{ trans('base.Join Now') }}</span>
                        </button>
                    </slot>
                </div>
...
export default {
  ...
  methods: {
    fetchEvents () {
      let $self = this
      request.get('/mobility/events?page=' + this.page + '&per_page=8')
        .then((response) => {
